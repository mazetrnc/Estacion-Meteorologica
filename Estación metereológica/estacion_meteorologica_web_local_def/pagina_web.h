  ...
  <section id="viento">
    ...
    <div class="grid">
      <div class="dato">
        ...
        <span class="dato-valor nivel-ok" id="vientoMs">--</span>
        <span class="dato-unidad">m/s</span>
      </div>
      <div class="dato">
        <span class="dato-label">velocidad</span>
        <span class="dato-valor" id="vientoKmh">--</span>
        <span class="dato-unidad">km/h</span>
      </div>
      <div class="dato">
        <span class="dato-label">velocidad</span>
        <span class="dato-valor" id="vientoNudos">--</span>
        <span class="dato-unidad">nudos</span>
      </div>
      <div class="dato">
        <span class="dato-label">sensor</span>
        <span class="dato-valor" id="vientoEstado">--</span>
      </div>
    </div>
  </section>
...
var UMBRALES = {
  ...
  viento:{ aviso: 8, alto: 14, extremo: 20.8 } // m/s (Beaufort: moderado/fuerte/temporal)
};
...
function generarAlertas(d) {
  ...
  var nW = getNivel(d.vientoMs, UMBRALES.viento);
  if (nW === "extremo") msgs.push({ c: "alerta-roja",    t: "viento muy fuerte", m: fmtNum(d.vientoMs) + " m/s - evita actividades al aire libre." });
  if (nW === "alto")    msgs.push({ c: "alerta-naranja", t: "viento fuerte",     m: fmtNum(d.vientoMs) + " m/s - precaucion con objetos sueltos." });
  if (nW === "aviso")   msgs.push({ c: "alerta-amarilla",t: "viento moderado",   m: fmtNum(d.vientoMs) + " m/s - puede dificultar actividades exteriores." });
...
function actualizar() {
  ...
  xhr.onload = function() {
    ...
    setEl("vientoMs",    d.vientoMs, 1);    setColor("vientoMs", d.vientoMs, UMBRALES.viento);
    setEl("vientoKmh",   d.vientoKmh, 1);
    setEl("vientoNudos", d.vientoNudos, 1);
    document.getElementById("vientoEstado").textContent = d.vientoOK ? "OK" : "sin datos";
    document.getElementById("vientoEstado").className =
      "dato-valor " + (d.vientoOK ? "nivel-ok" : "nivel-aviso");
