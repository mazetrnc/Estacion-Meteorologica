...
void setup() {
  ...
  if (!bme.begin(0x76, &Wire)) {
    Serial.println("[ERROR] BME280 no encontrado (SDA=15 SCL=27). Verifica cables.");
    while (1);
  }
  ...
  Serial.println("[OK] Anemometro RS485 configurado (RX=16, TX=17, DE=25, RE=26).");
...
void loop() {
  ...
  if (ahora - ultimaLectura >= INTERVALO_SERIAL_MS) {
    ...
    if (vientoOK) {
      Serial.print("  Velocidad:\t\t"); Serial.print(vientoMs, 1); Serial.println(" m/s");
    } else {
      Serial.print("  [ERROR] Codigo: 0x"); Serial.println(resultAnem, HEX);
    }
    ...
    if (millis() - lastPostMs >= POST_INTERVAL_MS) {
      ...
      if (WiFi.status() == WL_CONNECTED) {
        ...
        bool ok = sendToThingSpeak(temp, hum, pre, vientoMs, co2, tvoc, lluviaPorHora, uvi);
